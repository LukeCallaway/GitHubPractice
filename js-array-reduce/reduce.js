...
function extractValue(arr, key) {
    return arr.reduce(function(accum,nextIdx){
        accum.push(nextIdx[key]);
        return accum;
    },[]);
}
...
function vowelCount(str) {
    const vowels = "aeiou";
    return str.split('').reduce(function(accum, nextIdx){
        let lowerCased = nextIdx.toLowerCase()
...
/*
Write a function called partition which accepts an array and a callback and returns an array with two arrays inside of it. The partition function should run the callback 
function on each value in the array and if the result of the callback function at that specific value is true, the value should be placed in the first subarray. 
...
function partition(arr, callback) {
    return arr.reduce(function(accum, nextIdx){
        if(callback(nextIdx)){
            accum[0].push(nextIdx);
        }
        else{
            accum[1].push(nextIdx);
        }
        return accum;
    }, [[], []]);
}

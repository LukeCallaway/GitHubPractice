...
class Company {
  ...
  static async findAll(filters = {}) {
    ...
    if(minEmployees > maxEmployees){
      throw new ExpressError('MInimum Employees must be smaller number than Maximum Employee', 400)
    }
  ...
  static async getCompJobs(handle){
      const jobRes = await db.query(`select id, title, salary, equity, company_handle
      from jobs
      left join companies
      on companies.handle = jobs.company_handle
      where company_handle = 'hall-mills';`);

      const jobs = jobRes.rows;

      if(!jobs) throw new NotFoundError(`No jobs from company: ${handle}`);
